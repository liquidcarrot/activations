...
describe("Activation", () => {
  ...
  for (let i: number = 0; i < 100; i++) {
    ...
    it("Logistic value=" + value + "; derivative=false", () => {
      expect(Logistic(value, false)).closeTo(
        1 / (1 + Math.exp(-value)),
        deltaValue
      );
    });
    it("Logistic value=" + value + "; derivative=true", () => {
      expect(Logistic(value, true)).closeTo(
    ...
    it("TANH value=" + value + "; derivative=true", () => {
      expect(TANH(value, true)).closeTo(
        1 - Math.tanh(value) * Math.tanh(value),
        deltaValue
      );
    });
    ...
    it("ArSinH value=" + value + "; derivative=true", () => {
      expect(ArSinH(value, true)).closeTo(
        1 / Math.sqrt(value * value + 1),
        deltaValue
      );
    });

    it("SoftSign value=" + value + "; derivative=false", () => {
      expect(SoftSign(value, false)).closeTo(
        value / (1 + Math.abs(value)),
        deltaValue
      );
    });
    it("SoftSign value=" + value + "; derivative=true", () => {
      expect(SoftSign(value, true)).closeTo(
        1 / Math.pow(1 + Math.abs(value), 2),
        deltaValue
      );
    });
    ...
    it("LeakyRELU value=" + value + "; derivative=false", () => {
      expect(LeakyRELU(value, false)).closeTo(
        value <= 0 ? 0.01 * value : value,
        deltaValue
      );
    });
    ...
    it("SoftPlus value=" + value + "; derivative=false", () => {
      expect(SoftPlus(value, false)).closeTo(
        Math.log(1 + Math.exp(value)),
        deltaValue
      );
    });
    it("SoftPlus value=" + value + "; derivative=true", () => {
      expect(SoftPlus(value, true)).closeTo(
        1 / (1 + Math.exp(-value)),
        deltaValue
      );
    });
    ...
    it("BentIdentity value=" + value + "; derivative=false", () => {
      expect(BentIdentity(value, false)).closeTo(
        (Math.sqrt(value * value + 1) - 1) / 2 + value,
        deltaValue
      );
    });
    it("BentIdentity value=" + value + "; derivative=true", () => {
      expect(BentIdentity(value, true)).closeTo(
        value / (2 * Math.sqrt(value * value + 1)) + 1,
        deltaValue
      );
    });

    it("SiLU value=" + value + "; derivative=false", () => {
      expect(SiLU(value, false)).closeTo(
        value / (1 + Math.exp(-value)),
        deltaValue
      );
    });
    it("SiLU value=" + value + "; derivative=true", () => {
      expect(SiLU(value, true)).closeTo(
    ...
    it("Sinc value=" + value + "; derivative=false", () => {
      expect(Sinc(value, false)).closeTo(
        value === 0 ? 1 : Math.sin(value) / value,
        deltaValue
      );
    });
    it("Sinc value=" + value + "; derivative=true", () => {
      expect(Sinc(value, true)).closeTo(
        value === 0
          ? 0
    ...
    it("GAUSSIAN value=" + value + "; derivative=false", () => {
      expect(GAUSSIAN(value, false)).closeTo(
        Math.exp(-value * value),
        deltaValue
      );
    });
    it("GAUSSIAN value=" + value + "; derivative=true", () => {
      expect(GAUSSIAN(value, true)).to.be.closeTo(
        -2 * value * Math.exp(-value * value),
        deltaValue
      );
    });
  }
